...
public interface Aabb2fR
{
	...
	<T extends Tup2fC> T getSize(@ExtractionParam T res);
	float[] getSize(@ExtractionParam @MinLength(2) float[] res);
	
	float getCenterX();
	float getCenterY();
	
	<T extends Tup2fC> T getCenter(@ExtractionParam T res);
	...
	{
		float[] newMin = Vec2fUtils.minVector(getMinX(), getMinY(), Math.min(minX, maxX),  Math.min(minY, maxY), new float[2]);
	...
	{
		return scaleR(t.getV0(), t.getV1(), res);
	}
	...
	{
		return mergeR(minX, minY, max.getV0(), max.getV1(), res);
	}
