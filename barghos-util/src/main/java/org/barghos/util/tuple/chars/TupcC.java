...
public interface TupcC extends TupcR
{
	...
	/** {@inheritDoc}} */
	@Override
	default TupcC createNew(TupcR t)
	{
	 ...
	 *
	 * @param t An existing implementation of {@link TupcR} to adopt the
	 ...
	 * @return The current tuple.
	 */
	default TupcC setResize(TupcR t)
	{
		Validation.validateNotNull("t", t);
	...
	/** {@inheritDoc}} */
	@Override
	default TupcC resizeN(int size)
	{
		Validation.validateMin("size", size, 0);
		
		char[] oldValues = toArray();
	...
	/** {@inheritDoc}} */
	@Override
	default TupcC rearrangeN(int[] indices)
	{
		if(Validation.argumentValidation())
		{
	...
	/** {@inheritDoc}} */
	@Override
	default TupcC rearrangeResizeN(int[] indices)
	{
	...
	/** {@inheritDoc}} */
	@Override
	default TupcC swapByIndexN(int indexA, int indexB)
	{
		if(Validation.argumentValidation())
		{
	 ...
	 * @return This tuple.
	 */
	default TupcC rearrange(int[] indices)
	{
		if(Validation.argumentValidation())
		{
			Validation.notNull("indices", indices);
	 ...
	 * @return This tuple.
	 */
	default TupcC rearrangeResize(int[] indices)
	{
		Validation.validateNotNull("indices", indices);
	 ...
	 * @return This tuple.
	 */
	default TupcC swapByIndex(int indexA, int indexB)
	{
		if(Validation.argumentValidation())
		{
			int lastIndex = size() - 1;
