...
public interface TupdC extends TupdR
{
	...
	/** {@inheritDoc}} */
	@Override
	default TupdC createNew(TupdR t)
	{
	 ...
	 *
	 * @param t An existing implementation of {@link TupdR} to adopt the
	 ...
	 * @return The current tuple.
	 */
	default TupdC setResize(TupdR t)
	{
		Validation.validateNotNull("t", t);
	 ...
	 * @return The current tuple.
	 */
	default TupdC setArray(double... t)
	{
	...
	/** {@inheritDoc}} */
	@Override
	default TupdC resizeN(int size)
	{
		Validation.validateMin("size", size, 0);
		
		double[] oldValues = toArray();
	...
	/** {@inheritDoc}} */
	@Override
	default TupdC rearrangeN(int[] indices)
	{
		if(Validation.argumentValidation())
		{
	...
	/** {@inheritDoc}} */
	@Override
	default TupdC rearrangeResizeN(int[] indices)
	{
	...
	/** {@inheritDoc}} */
	@Override
	default TupdC swapByIndexN(int indexA, int indexB)
	{
		if(Validation.argumentValidation())
		{
	 ...
	 * @return This tuple.
	 */
	default TupdC rearrange(int[] indices)
	{
		if(Validation.argumentValidation())
		{
			Validation.notNull("indices", indices);
	 ...
	 * @return This tuple.
	 */
	default TupdC rearrangeResize(int[] indices)
	{
		Validation.validateNotNull("indices", indices);
	 ...
	 * @return This tuple.
	 */
	default TupdC swapByIndex(int indexA, int indexB)
	{
		if(Validation.argumentValidation())
		{
			int lastIndex = size() - 1;
