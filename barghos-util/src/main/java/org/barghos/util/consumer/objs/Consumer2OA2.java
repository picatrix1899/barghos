...
public interface Consumer2OA2 extends Consumer2<Object[][],Object[][]>
{
     ...
     * operation after.
     */
    default Consumer2OA2 thenObject(Consumer2OA2 after)
    {
	...
	default Consumer2OA2 thenObject(Consumer2OA2... after)
    {
    	...
    	return (a, b) -> {acceptObjectArray(a, b); for(Consumer2OA2 consumer : after) consumer.acceptObjectArray(a, b);};
    }
     ...
     * 
     * @return A new {@link Consumer2OA2} performing the operation before and this
     * operation.
     */
    default Consumer2OA2 beforeObject(Consumer2OA2 before)
    {
     ...
     * 
     * @return A new {@link Consumer2OA2} performing the operations before and
     * this operation.
     */
    default Consumer2OA2 beforeObject(Consumer2OA2... before)
    {
    	...
    	return (a, b) -> {for(Consumer2OA2 consumer : before) consumer.acceptObjectArray(a, b); acceptObjectArray(a, b);};
    }
     ...
     */
	static Consumer2OA2 sequenceObject(Consumer2OA2... consumers)
    {
    	...
    	return (a, b) -> {for(Consumer2OA2 consumer : consumers) consumer.acceptObjectArray(a, b);};
    }
    ...
    default Consumer2OA2 then(Consumer2<Object[][],Object[][]> after)
    {
		 ...
		 */
    	if(after instanceof Consumer2OA2)
    	{
    		final Consumer2OA2 originalAfter = (Consumer2OA2)after;
	...
	default Consumer2OA2 then(Consumer2<Object[][],Object[][]>... after)
    {
    	...
    	if(after.length == 1)
    	{
    		 ...
    		 */
    		if(after[0] instanceof Consumer2OA2)
        	{
        		final Consumer2OA2 originalAfter = (Consumer2OA2)after[0];
    	...
    	return (a, b) -> {
    		...
    		{
    			if(consumer instanceof Consumer2OA2)
    				((Consumer2OA2)consumer).acceptObjectArray(a, b);
    			else
    				consumer.accept(a, b);
    		}
    	};
    ...
    default Consumer2OA2 before(Consumer2<Object[][],Object[][]> before)
    {
		 ...
		 * the desired type to avoid boxing.
		 */
    	if(before instanceof Consumer2OA2)
    	{
    		final Consumer2OA2 originalBefore = (Consumer2OA2)before;
    ...
    default Consumer2OA2 before(Consumer2<Object[][],Object[][]>... before)
    {
    	...
    	if(before.length == 1)
    	{
    		 ...
    		 * as the desired type to avoid boxing.
    		 */
    		if(before[0] instanceof Consumer2OA2)
        	{
        		final Consumer2OA2 originalBefore = (Consumer2OA2)before[0];
     ...
     */
    @SuppressWarnings("unchecked")
	static Consumer2OA2 sequence(Consumer2<Object[][],Object[][]>... consumers)
    {
    	Validation.validateNotNull("consumers", consumers);
    	Validation.validateEntriesNotNull("consumers", consumers);
    	...
    	{
    		if(consumers[0] instanceof Consumer2OA2)
    			return (Consumer2OA2) consumers[0];
    		else
    			return (Consumer2OA2) consumers[0]::accept;
    	}
