...
public interface Consumer2SA2 extends Consumer2<short[][],short[][]>
{
     ...
     * operation after.
     */
    default Consumer2SA2 andThenShort(Consumer2SA2 after)
    {
	...
	default Consumer2SA2 andThenShort(Consumer2SA2... after)
    {
    	...
    	return (a, b) -> {acceptShortArray(a, b); for(Consumer2SA2 consumer : after) consumer.acceptShortArray(a, b);};
    }
     ...
     * 
     * @return A new {@link Consumer2SA2} performing the operation before and this
     * operation.
     */
    default Consumer2SA2 beforeThatShort(Consumer2SA2 before)
    {
     ...
     * 
     * @return A new {@link Consumer2SA2} performing the operations before and
     * this operation.
     */
    default Consumer2SA2 beforeThatShort(Consumer2SA2... before)
    {
    	...
    	return (a, b) -> {for(Consumer2SA2 consumer : before) consumer.acceptShortArray(a, b); acceptShortArray(a, b);};
    }
     ...
     */
	static Consumer2SA2 inSequenceShort(Consumer2SA2... consumers)
    {
    	...
    	return (a, b) -> {for(Consumer2SA2 consumer : consumers) consumer.acceptShortArray(a, b);};
    }
     ...
     * operation after.
     */
    default Consumer2SA2 andThen(Consumer2<short[][],short[][]> after)
    {
    	Validation.validateNotNull("after", after);
		 ...
		 */
    	if(after instanceof Consumer2SA2)
    	{
    		final Consumer2SA2 originalAfter = (Consumer2SA2)after;
	...
	default Consumer2SA2 andThen(Consumer2<short[][],short[][]>... after)
    {
    	...
    	if(after.length == 1)
    	{
    		 ...
    		 */
    		if(after[0] instanceof Consumer2SA2)
        	{
        		final Consumer2SA2 originalAfter = (Consumer2SA2)after[0];
    	...
    	return (a, b) -> {
    		...
    		{
    			if(consumer instanceof Consumer2SA2)
    				((Consumer2SA2)consumer).acceptShortArray(a, b);
    			else
    				consumer.accept(a, b);
    		}
    	};
     ...
     * this operation.
     */
    default Consumer2SA2 beforeThat(Consumer2<short[][],short[][]> before)
    {
    	Validation.validateNotNull("before", before);
		 ...
		 * the desired type to avoid boxing.
		 */
    	if(before instanceof Consumer2SA2)
    	{
    		final Consumer2SA2 originalBefore = (Consumer2SA2)before;
    ...
    default Consumer2SA2 beforeThat(Consumer2<short[][],short[][]>... before)
    {
    	...
    	if(before.length == 1)
    	{
    		 ...
    		 * as the desired type to avoid boxing.
    		 */
    		if(before[0] instanceof Consumer2SA2)
        	{
        		final Consumer2SA2 originalBefore = (Consumer2SA2)before[0];
     ...
     */
    @SuppressWarnings("unchecked")
	static Consumer2SA2 inSequence(Consumer2<short[][],short[][]>... consumers)
    {
    	Validation.validateNotNull("consumers", consumers);
    	Validation.validateEntriesNotNull("consumers", consumers);
    	...
    	{
    		if(consumers[0] instanceof Consumer2SA2)
    			return (Consumer2SA2) consumers[0];
    		else
    			return (Consumer2SA2) consumers[0]::accept;
    	}
